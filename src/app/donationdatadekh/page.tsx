...
const downloadAsPDF = async (formDataList: DonationData[]) => {
  ...
  for (let i = 0; i < formDataList.length; i++) {
    ...
    autoTable(doc, {
      head: [["Sr. No.", "Name", "Email", "Contact Number", "Amount"]],
      body: tableData,
      startY: currentY,
      theme: "grid",
    });
...
const Page: React.FC = () => {
  ...
  const exportToExcel = () => {
    const filteredData = formDataList.map(
      ({ serial, name, email, PhoneNumber, Amount }) => ({
        "Sr. No.": serial,
        "Name": name,
        "Email": email,
